package task

import (
	"encoding/json"
	"errors"
	"fmt"
	"os"
	"sync"
	"time"
)

const filePath = "sources/tasks.json"

var ErrNotFound = errors.New("task not found")

type Repo struct {
	mu    sync.RWMutex
	seq   int64
	items map[int64]*Task
}

func NewRepo() *Repo {
	r := &Repo{items: make(map[int64]*Task)}
	_ = r.LoadFromFile()
	return r
}

func (r *Repo) List() []*Task {
	r.mu.RLock()
	defer r.mu.RUnlock()
	out := make([]*Task, 0, len(r.items))
	for _, t := range r.items {
		out = append(out, t)
	}
	return out
}

func (r *Repo) Get(id int64) (*Task, error) {
	r.mu.RLock()
	defer r.mu.RUnlock()
	t, ok := r.items[id]
	if !ok {
		return nil, ErrNotFound
	}
	return t, nil
}

func (r *Repo) Create(title string) *Task {
	r.mu.Lock()
	defer r.mu.Unlock()
	r.seq++
	now := time.Now()
	t := &Task{ID: r.seq, Title: title, CreatedAt: now, UpdatedAt: now, Done: false}
	r.items[t.ID] = t
	_ = r.SaveToFile() // сохраняем
	return t
}

func (r *Repo) Update(id int64, title string, done bool) (*Task, error) {
	r.mu.Lock()
	defer r.mu.Unlock()
	t, ok := r.items[id]
	if !ok {
		return nil, ErrNotFound
	}
	t.Title = title
	t.Done = done
	t.UpdatedAt = time.Now()
	_ = r.SaveToFile()
	return t, nil
}

func (r *Repo) Delete(id int64) error {
	//r.mu.Lock()
	//defer r.mu.Unlock()
	if _, ok := r.items[id]; !ok {
		return ErrNotFound
	}
	delete(r.items, id)
	_ = r.SaveToFile()
	return nil
}

func (r *Repo) LoadFromFile() error {
	r.mu.Lock()
	defer r.mu.Unlock()

	file, err := os.Open(filePath)
	if err != nil {
		if os.IsNotExist(err) {
			r.items = map[int64]*Task{}
			r.seq = 0
			return nil
		}
		fmt.Println("Error creating file:", err)
		return err
	}
	defer file.Close()

	if err := json.NewDecoder(file).Decode(&r.items); err != nil {
		return err
	}

	// вычисляем nextID
	var maxID int64
	for _, t := range r.items {
		if t.ID > maxID {
			maxID = t.ID
		}
	}
	r.seq = maxID
	return nil
}

func (r *Repo) SaveToFile() error {
	//r.mu.Lock()
	//defer r.mu.Unlock()

	file, err := os.Create(filePath)
	if err != nil {
		fmt.Println("Error creating file:", err)
		return err
	}
	defer file.Close()

	encoder := json.NewEncoder(file)
	encoder.SetIndent("", "  ") // красиво форматировать
	return encoder.Encode(r.items)
}
